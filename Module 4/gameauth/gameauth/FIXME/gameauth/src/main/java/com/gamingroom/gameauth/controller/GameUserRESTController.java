...
// These annotations define the base path for all resources in this controller and specify that responses will be in JSON format.
@Path ("/gameusers")
...
public class GameUserRESTController {
    ...
    // These annotations ensure that only authenticated users with specific roles can access these methods, enhancing security.
    @RolesAllowed ({ "USER" })
    @GET
    @Path("/{id}")
